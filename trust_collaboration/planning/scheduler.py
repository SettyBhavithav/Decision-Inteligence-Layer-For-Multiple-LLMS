...
class DynamicTaskScheduler:
    ...
    def get_ready_tasks(self) -> List[Dict[str, Any]]:
        ...
        for task_id, deps in self.dependencies.items():
            if task_id in self.completed_tasks or task_id in self.running_tasks:
                continue
            # If all dependencies are in the completed set, the task is ready
    ...
    def get_execution_order(self) -> List[str]:
        """
        Performs static topological sort to estimate an execution order.
        Returns list of task IDs. Raises ValueError if a cycle is detected.
        """
